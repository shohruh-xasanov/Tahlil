...
const getMain = {
  mainMenu: async (req, res) => {
    ...
    const lastnews = await News.find({['name.eng']:{$gte:0}})
      ...
      .sort({ createdAt: -1 })
      .populate("categoryID");
    const actual = await News.find({['name.eng']:{$gte:0}})
      .limit(6)
      .sort({ createdAt: -1 })
      .populate("categoryID");
    ...
    const newsCat1 = await News.find({categoryID: id1}).and({ ["name.eng"]: { $gte: 0 } }).sort({createdAt:-1}).limit(8).populate('categoryID')
